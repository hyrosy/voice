...
import {
  ...
  Users,
  AlertTriangle,
} from "lucide-react"; // <-- Add Search here
...
const AdminDashboardPage = () => {
  ...
  const awaitingAdminConfirmation = orders.filter(
    (o) => o.status === "Awaiting Admin Confirmation"
  ).length;
  const inProgressOrders = orders.filter(
    (o) => o.status === "In Progress" || o.status === "Pending Approval"
  ...
  const handlePaymentApproval = async (orderId: string) => {
    ...
    if (error) {
      ...
      if (orderToApprove.actors?.ActorEmail) {
        ...
        console.warn(
          "Could not send notification: Actor email missing for order",
          orderToApprove.order_id_string
        );
  ...
  const filteredAndSortedOrders = useMemo(() => {
    ...
    if (filterPayment !== "all") {
      processedOrders = processedOrders.filter(
        (order) => order.payment_method === filterPayment
      );
    }
    if (filterSearchTerm.trim() !== "") {
      ...
      processedOrders = processedOrders.filter(
        (order) =>
          ...
          (order.actors?.ActorName &&
            order.actors.ActorName.toLowerCase().includes(searchTermLower))
      );
    }
  ...
  return (
    <div className="min-h-screen bg-background p-4 md:p-8 text-foreground">
      <div className="max-w-7xl mx-auto">
        ...
        {message && (
          <p className="mb-4 p-3 bg-slate-700 rounded-md text-sm">{message}</p>
        )}
        {/* --- NEW: Filter Controls --- */}
        <div className="mb-6 p-4 bg-card rounded-lg border border flex flex-col sm:flex-row gap-4 items-center">
          ...
          <div className="relative flex-grow">
            ...
            <Search
              size={16}
              className="absolute left-2.5 top-1/2 transform -translate-y-1/2 text-slate-500"
            />
            {filterSearchTerm && (
              <button
          ...
          <table className="w-full text-left min-w-[900px]">
            ...
            <thead className="bg-slate-700 text-xs uppercase text-muted-foreground">
              <tr>
                ...
                <th
                  className="p-4 cursor-pointer hover:bg-slate-600 transition-colors text-right"
                  onClick={() => handleSort("total_price")}
                >
                  {" "}
                  {/* Added text-right */}
                  <span className="flex items-center justify-end gap-1">
                    Price (MAD) <ArrowUpDown size={12} />
                  </span>{" "}
                  {/* Added justify-end */}
                </th>
            ...
            <tbody>
              ...
              {filteredAndSortedOrders.map((order) => (
                ...
                <tr
                  key={order.id}
                  className={`border-b border text-sm ${
                    order.status === "Awaiting Admin Confirmation"
                      ? "bg-yellow-900/30 hover:bg-yellow-900/50" // Highlight this status
                      : order.status === "Awaiting Actor Confirmation"
                      ? "bg-blue-900/30 hover:bg-blue-900/50" // Different highlight for actor
                      : "hover:bg-accent/50"
                  }`}
                ...
                >
                  ...
                  <td className="p-4 text-muted-foreground">
                    {order.actors?.ActorName || "N/A"}
                  </td>
                  <td className="p-4 text-muted-foreground text-right">
                    {order.total_price?.toFixed(2) || "N/A"}
                  </td>
                  ...
                  <td className="p-4" onClick={(e) => e.stopPropagation()}>
                    ...
                    >
                      ...
                      <option value="Awaiting Admin Confirmation">
                        Awaiting Admin Confirmation
                      </option>
                      <option value="Awaiting Actor Confirmation">
                    ...
                    {order.status === "Awaiting Admin Confirmation" &&
                      order.payment_method === "bank" && (
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            handlePaymentApproval(order.id);
                          }}

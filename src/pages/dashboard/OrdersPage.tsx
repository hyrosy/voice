...
import { 
  Loader2, Package, Phone, ShoppingBag, Eye, User, Copy, Globe, Filter
} from 'lucide-react';
import {
...
const OrdersPage = () => {
  ...
  useEffect(() => {
      if(selectedSiteId === 'all') {
          ...
          setFilteredOrders(allOrders.filter(o => o.portfolio_id === selectedSiteId));
      }
  ...
  const updateStatus = async (orderId: string, newStatus: string) => {
    // Optimistic Update
    const updater = (prev: ProOrder[]) => prev.map(o => o.id === orderId ? { ...o, status: newStatus as any } : o);
    ...
    await supabase.from('pro_orders').update({ status: newStatus }).eq('id', orderId);
  };

  const saveNotes = async () => {
    if (!selectedOrder) return;
    setIsSaving(true);
    const { error } = await supabase.from('pro_orders').update({ notes: internalNotes }).eq('id', selectedOrder.id);
    if (!error) {
        const updater = (prev: ProOrder[]) => prev.map(o => o.id === selectedOrder.id ? { ...o, notes: internalNotes } : o);
        setAllOrders(updater);
        setFilteredOrders(updater);
        ...
        <CardContent className="p-0">
            {filteredOrders.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-64 text-muted-foreground">
                    ...
                    <p>No orders found {selectedSiteId !== 'all' && 'for this website'}.</p>
                </div>
            ) : (
                        ...
                        <TableBody>
                            {filteredOrders.map((order) => (
                                <TableRow key={order.id} className="cursor-pointer hover:bg-muted/50" onClick={() => handleViewDetails(order)}>
                                    ...
                                    <TableCell>
                                        <span className="font-medium text-sm">{order.quantity}x {order.product_name}</span>
                                    </TableCell>
                ...
                <>
                    <SheetHeader className="mb-6">
                        <SheetTitle className="flex items-center gap-2">
                            Order Details
                            <Badge variant="secondary">{getSiteName(selectedOrder.portfolio_id)}</Badge>
                        </SheetTitle>
                        <SheetDescription>Placed on {new Date(selectedOrder.created_at).toLocaleString()}</SheetDescription>
                    </SheetHeader>
                    ...
                    <div className="space-y-8">
                        <div className="space-y-4">
                            ...
                            <div className="bg-muted/30 p-4 rounded-lg space-y-3 border">
                                <div><Label className="text-xs text-muted-foreground">Name</Label><div className="font-medium">{selectedOrder.customer_name}</div></div>
                                <div className="grid grid-cols-2 gap-4">
                                    <div><Label className="text-xs text-muted-foreground">Phone</Label><div className="font-medium">{selectedOrder.customer_phone}</div></div>
                                    <div><Label className="text-xs text-muted-foreground">Address</Label><div className="font-medium text-sm">{selectedOrder.customer_address}</div></div>
                                </div>
                            </div>
                        </div>
                        {/* Status Management */}
                        ...
                        <div className="space-y-2">
                            <Label>Notes</Label>
                            <Textarea value={internalNotes} onChange={(e) => setInternalNotes(e.target.value)} className="bg-background"/>
                            <Button size="sm" onClick={saveNotes} disabled={isSaving}>{isSaving ? <Loader2 className="animate-spin h-4 w-4"/> : "Save"}</Button>
                        </div>
                    </div>
                </>
            )}

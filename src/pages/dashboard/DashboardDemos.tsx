...
import  {  Music,  Trash2,  PencilLine,  Video,  UploadCloud,  RefreshCw,  Save,  Mic,  CheckCircle,  Library  }  from  'lucide-react';
//  ---
...
   //  ---  MODIFIED:  Fetch  data  for  this  page  ---
...
         //  ---  NEW:  5.  Fetch  actor  recordings  (library)  ---
...
         setRecordings(recordingsData  as  ActorRecording[]);
         //  ---  END  NEW  ---
...
      e.preventDefault();
      //  General  validation
      if  (!newDemo.title  ||  !newDemo.language  ||  !newDemo.style_tag  ||  !layoutActorData.id)  {
         setMessage("Please  fill  all  demo  fields  (Title,  Language,  Style).");
         return;
      }
...
   //  (All  Script  and  Video  handlers  are  unchanged)
...
                        <CardDescription>Upload  a  new  file  or  add  from  your  library.</CardDescription>
                     </CardHeader>
                     <CardContent>
...
                           {/*  ---  NEW:  Source  Selector  ---  */}
                           <RadioGroup  value={uploadSource}  onValueChange={(v)  =>  setUploadSource(v  as  'upload'  |  'library')}  className="grid  grid-cols-2  gap-4">
                              <div>
                                 <Label  htmlFor="upload"  className={`flex  flex-col  items-center  justify-center  p-4  border  rounded-lg  cursor-pointer  ${uploadSource  ===  'upload'  ?  'border-primary'  :  ''}`}>
...
                              </div>
                              <div>
                                 <Label  htmlFor="library"  className={`flex  flex-col  items-center  justify-center  p-4  border  rounded-lg  cursor-pointer  ${uploadSource  ===  'library'  ?  'border-primary'  :  ''}`}>
                                    <RadioGroupItem  value="library"  id="library"  className="sr-only"  />
...
                                       {recordings.length  ===  0  &&  <SelectItem  value="none"  disabled>Your  library  is  empty.</SelectItem>}
                                       {recordings.map(rec  =>  {
                                      const  url  =  rec.cleaned_audio_url  ||  rec.raw_audio_url;
                                          return  (
                                             <SelectItem  key={rec.id}  value={url}>
                                                <div  className="flex  flex-col">
                                                   <span  className="font-medium">{rec.name}</span>
                                                   {rec.cleaned_audio_url  ?  (
                                                  <span  className="text-xs  text-green-500">AI  Cleaned</span>
                                                   )  :  (
                                                      <span  className="text-xs  text-yellow-500">Raw  Audio</span>
                                   )
                                                   }
                                                </div>
                                             </SelectItem>

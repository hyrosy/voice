...
const Hero: React.FC<BlockProps> = ({ data }) => {
  ...
  return (
    <section className="relative min-h-[100dvh] flex flex-col justify-center overflow-hidden bg-black text-white">
          ...
          <div className="absolute inset-0 z-0">
             <div className="absolute inset-0 overflow-hidden">
                <img 
                   ...
                   alt="Hero Background" 
                   // FIX 1: Add 'will-change-transform' to tell the browser to prepare the GPU
       ...
       {/* --- LAYER 2: CONTENT --- */}
       {/* FIX 5: Added 'mx-auto w-full' to ensure the container itself is centered in the viewport */}
       <div className={cn("relative z-10 container mx-auto w-full h-full flex flex-col justify-center px-6 pt-20", alignmentClass)}>
          <div className={cn("space-y-6 md:space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-1000 fill-mode-both", 
             data.alignment === 'center' ? 'max-w-4xl mx-auto' : 'max-w-3xl' // Added mx-auto here too for safety
          )}>
             ...
             {/* Headline */}
             <h1 className="text-5xl md:text-7xl lg:text-8xl font-bold tracking-tight leading-[1.1] md:leading-[1.1] text-transparent bg-clip-text bg-gradient-to-b from-white to-white/60 drop-shadow-sm will-change-transform">
                {data.headline}
             </h1>
             ...
             {data.ctaText && (
                <div className="pt-4 md:pt-8">
                    <Button 
                        asChild 
                        size="lg" 
                        // FIX 4: Removed complex shadow on mobile
                        className="h-14 md:h-16 px-8 md:px-10 rounded-full bg-white text-black hover:bg-neutral-200 text-base md:text-lg font-semibold transition-all hover:scale-105 shadow-none md:shadow-[0_0_40px_-10px_rgba(255,255,255,0.3)] group"
                    >

...
const Hero: React.FC<any> = ({ data, id, isPreview }) => {
  ...
  return (
    <section className="relative min-h-[100dvh] flex flex-col justify-center overflow-hidden bg-black text-white">
      ...
      <div
        className={cn(
          "relative z-10 container mx-auto w-full h-full flex flex-col justify-center px-6 pt-20",
          alignmentClass
        )}
      >
        <div
          className={cn(
            "space-y-6 md:space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-1000 fill-mode-both",
        ...
        >
          ...
          <div className="overflow-hidden">
            <span className="inline-block py-1 px-3 rounded-full border border-primary/30 bg-primary/10 backdrop-blur-sm text-xs md:text-sm font-medium tracking-widest uppercase text-primary animate-fade-up">
              {/* 🚀 3. REPLACE TEXT WITH INLINE EDIT */}
              <InlineEdit
                tagName="span"
                text={data.label || "Welcome"}
                sectionId={id}
                fieldKey="label"
                isPreview={isPreview}
              />
          ...
          {/* Headline */}
          {/* 🚀 3. REPLACE H1 WITH INLINE EDIT */}
          <InlineEdit
            tagName="h1"
            className="text-5xl md:text-7xl lg:text-8xl font-bold tracking-tight leading-[1.1] md:leading-[1.1] text-transparent bg-clip-text bg-gradient-to-b from-primary to-primary/60 drop-shadow-sm will-change-transform block"
          ...
          {(data.ctaText || data.secondaryCtaText) && (
            <div
              className={cn(
                "pt-4 md:pt-8 flex flex-wrap gap-4",
      ...
      <div className="absolute bottom-8 left-0 right-0 z-20 flex justify-center animate-bounce-slow opacity-70">
        <a
          href="#content"
          className="flex flex-col items-center gap-2 text-primary/50 hover:text-primary transition-colors cursor-pointer"
        >

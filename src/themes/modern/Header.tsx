...
const Header: React.FC<BlockProps> = ({ data, allSections, isPreview }) => {
  ...
  // --- SCROLL DETECTION ---
  // This detects if the user has scrolled down to toggle the "Glass" effect
  useEffect(() => {
    const handleScroll = () => {
  ...
  return (
    ...
    <header 
        className={cn(
            ...
            // If sticky is enabled AND scrolled: Show Glass. Otherwise: Transparent.
    ...
    {/* This sits outside the header tag to cover the whole screen */}
    <div className={cn(
        "fixed inset-0 z-[55] bg-neutral-950 md:hidden flex flex-col items-center justify-center space-y-8 transition-all duration-500 ease-in-out",
        ...
        <div className="flex flex-col items-center gap-6 relative z-10 w-full px-8">
            {menuItems.map((item, idx) => (
                <button 
                    ...
                    style={{ transitionDelay: `${idx * 50}ms` }} // Staggered animation
                >
                    {item.label}
                </button>
            ))}

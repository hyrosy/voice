...
// 1. SCHEMA: Defines the UI Settings Panel for the Actor
export const schema = [
  {
    id: "logoText",
    type: "string",
...
// 2. COMPONENT: Taking `id` and `isPreview` so the SDK knows if it's in the builder!
export default function Header({ data, id, isPreview }: any) {
  const [isScrolled, setIsScrolled] = useState(false);
  const [isHidden, setIsHidden] = useState(false);
  const [lastScrollY, setLastScrollY] = useState(0);
  const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false);

  // 🚀 3. NAVIGATION: Use the platform-provided customLinks, fallback for Studio dev mode
  const navLinks = data.customLinks || [
    { label: "Home", url: "/" },
    { label: "Features", url: "#" },
    { label: "Pricing", url: "#" },
  ];
  ...
  return (
    <>
      ...
      <header
        className={cn(
          "fixed top-6 left-0 right-0 z-50 transition-all duration-500 ease-[cubic-bezier(0.32,0.72,0,1)]",
      ...
      >
        <div
          className={cn(
            ...
            isScrolled
              ? "max-w-4xl bg-white/70 shadow-lg rounded-full py-3 border border-white/20"
              : "max-w-7xl py-6 bg-transparent"
          )}
          // 🚀 Showcasing how to use the schema slider data dynamically!
          style={{
            backdropFilter: isScrolled
              ? `blur(${
                  data.glassStrength !== undefined ? data.glassStrength : 12
                }px)`
              : "none",
        ...
        >
          ...
          <div className="font-semibold tracking-tight text-lg text-black">
            {data.logoUrl ? (
              ...
              // 🚀 4. INLINE EDITING: The dev just uses UCP.Text!
              <UCP.Text
                as="span"
                field="logoText"
                default="MY BRAND"
          ...
          <nav className="flex flex-col gap-6 text-center">
            {navLinks.map((link: any, idx: number) => (
              <a
                key={idx}
                href={link.url}
                className="text-3xl font-bold tracking-tight text-black hover:text-blue-600 transition-colors"
              >
                {link.label}
              </a>
            ))}
...
// Attach the schema to the component so the platform can read it

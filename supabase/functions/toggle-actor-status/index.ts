...
// 🚀 1. DEFINE CORS INLINE (Prevents deployment import crashes)
const corsHeaders = {
...
serve(async (req) => {
  ...
  try {
    ...
    if (!actorId || typeof newStatus !== "boolean") {
      throw new Error(
        "Invalid input: actorId (string) and newStatus (boolean) are required."
      );
    }
    ...
    const { data: updatedActor, error: updateError } = await supabaseAdmin
      .from("actors")
      .update({ IsActive: newStatus }) // 💡 Make sure this matches your exact DB column casing
      .eq("id", actorId)
      .select('id, "IsActive"')
      .single();
    ...
    console.log(
      `Actor ${actorId} status updated successfully to ${updatedActor.IsActive}.`
    );
    ...
    return new Response(
      JSON.stringify({
        ...
        isActive: updatedActor.IsActive,
      }),
      {
    ...
    return new Response(JSON.stringify({ error: error.message }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
      status: error.message.startsWith("Permission denied")
        ? 403
